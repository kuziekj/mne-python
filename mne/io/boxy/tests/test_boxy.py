...
def test_boxy_load():
    ...
    mne_dc = mne.io.read_raw_boxy(boxy_raw_dir, 'DC', multi_file=False,
                                  verbose=True).load_data()._data
    mne_ac = mne.io.read_raw_boxy(boxy_raw_dir, 'AC', multi_file=False,
                                  verbose=True).load_data()._data
    mne_ph = mne.io.read_raw_boxy(boxy_raw_dir, 'Ph', multi_file=False,
                                  verbose=True).load_data()._data
    ...
    assert (abs(ppod_ac - mne_ac) <= thresh).all()
    assert (abs(ppod_dc - mne_dc) <= thresh).all()
    assert (abs(ppod_ph - mne_ph) <= thresh).all()
